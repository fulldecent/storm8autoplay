...
function action {
  ...
  CHOICE=$(wget -nv -U "$UA" --output-document - --header "$COOKIE" "http://$SERVER.storm8.com/investment.php" | ./realestateselect.pl)
  while [ "$CHOICE" ]
  do
    CHOICE=$(wget -nv -U "$UA" --output-document - --header "$COOKIE" "http://$SERVER.storm8.com/investment.php?action=buy&inv_id=$CHOICE" | ./realestateselect.pl)
  done
}
